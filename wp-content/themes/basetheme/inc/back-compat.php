...
function thebase_get_insufficient_requirements_message() {
	...
	$insufficient_wp  = version_compare( $wp_version, basetheme_MINIMUM_WP_VERSION, '<' );
	$insufficient_php = version_compare( phpversion(), basetheme_MINIMUM_PHP_VERSION, '<' );
	...
	if ( $insufficient_wp && $insufficient_php ) {
		...
		return sprintf( __( 'basetheme requires at least WordPress version %1$s and PHP version %2$s. You are running versions %3$s and %3$s respectively. Please update and try again.', 'basetheme' ), basetheme_MINIMUM_WP_VERSION, basetheme_MINIMUM_PHP_VERSION, $wp_version, phpversion() );
	}

	if ( $insufficient_wp ) {
		/* translators: 1: required WP version number, 2: available WP version number */
		return sprintf( __( 'basetheme requires at least WordPress version %1$s. You are running version %2$s. Please update and try again.', 'basetheme' ), basetheme_MINIMUM_WP_VERSION, $wp_version );
	}

	if ( $insufficient_php ) {
		/* translators: 1: required PHP version number, 2: available PHP version number */
		return sprintf( __( 'basetheme requires at least PHP version %1$s. You are running version %2$s. Please update and try again.', 'basetheme' ), basetheme_MINIMUM_PHP_VERSION, phpversion() );
	}
