...
class Component implements Component_Interface, Templating_Component_Interface {
	...
	public function filter_archive_title( $title ) {
		...
		if ( is_home() && is_front_page() ) {
			...
			$title = sprintf( esc_html__( 'Author: %s', 'basetheme' ), get_the_author() );
		} elseif ( is_day() ) {
			/* translators: 1: Day */
			$title = sprintf( esc_html__( 'Day: %s', 'basetheme' ), get_the_date() );
		} elseif ( is_month() ) {
			/* translators: 1: Month */
			$title = sprintf( esc_html__( 'Month: %s', 'basetheme' ), get_the_date( 'F Y' ) );
		} elseif ( is_year() ) {
			/* translators: 1: Year */
			$title = sprintf( esc_html__( 'Year: %s', 'basetheme' ), get_the_date( 'Y' ) );
