	...
	render() {
		...
		return (
			<div className={'thebase-font-pair-wrap'}>
				<Button className={ 'thebase-font-pair-btn' } label={ __( 'Choose a font', 'basetheme' ) } onClick={ () => { this.state.isVisible ? toggleClose() : toggleVisible() } }>
					<Dashicon icon="portfolio" />
				</Button>
				{ this.state.isVisible && (
					<Popover position="bottom left" className="thebase-font-pair-popover" onClose={ toggleClose }>
						<h2 style={{ textAlign:'center' }}>{ __( 'Select a Font Pairing', 'basetheme' ) }</h2>
						<ButtonGroup className="tb-font-pair-group" aria-label={ __( 'Select a Font Pair', 'basetheme' ) }>
							{ map( this.state.fonts, ( { hfont, bfont, hv, img, name } ) => {
								return (
									this.state.confirm === name ? (
										...
										>
											{ __( 'Confirm Change Settings?', 'basetheme' ) }
										</Button>
									) : (
										<Button
