...
class Theme {
	...
	/**
	 * Main TheBase\Theme Class Instance.
	 *
			...
			if ( ! $component instanceof Component_Interface ) {
				throw new InvalidArgumentException(
					sprintf(
						...
						__( 'The theme component %1$s does not implement the %2$s interface.', 'basetheme' ),
						gettype( $component ),
	...
	public function __clone() {
		...
		_doing_it_wrong( __FUNCTION__, esc_html__( 'Something went wrong.', 'basetheme' ), '1.0' );
	}
	...
	public function component( string $slug ) : Component_Interface {
		if ( ! isset( $this->components[ $slug ] ) ) {
			throw new InvalidArgumentException(
				sprintf(
					/* translators: %s: slug */
					__( 'No theme component with the slug %s exists.', 'basetheme' ),
					$slug
				)
			);
		}
