	...
	public function get_breadcrumb( $args = array() ) {
		$this->args = apply_filters(
			...
			wp_parse_args(
				$args,
				array(
					'home_title'     => __( 'Home', 'basetheme' ),
					'404_title'      => __( 'Error 404', 'basetheme' ),
					'search_title'   => __( 'Search results for', 'basetheme' ),
					'page'           => __( 'Page', 'basetheme' ),
					'show_shop'      => true,
					'show_title'     => true,
					'color_style'    => '',
					'blog_id'        => '',
		...
		$this->settings = wp_parse_args(
			...
			array(
				...
				'after'            => '</span>',
				'home_link'        => home_url( '/' ),
				'wrap_before'      => '<nav id="thebase-breadcrumbs" aria-label="' . esc_attr__( 'Breadcrumbs', 'basetheme' ) . '"  class="thebase-breadcrumbs"><div class="thebase-breadcrumb-container"' . ( $this->args['color_style'] ? ' style="' . esc_attr( $this->args['color_style'] ) . '"' : '' ) . '>',
				'wrap_after'       => '</div></nav>',
		...
		$this->post_types = wp_parse_args(
			...
			array(
				...
				'ht_kb'      => array(
					...
					'archive_label' => esc_html__( 'Knowledge Base', 'basetheme' ),
				),
			)
		);
		$html = '';
