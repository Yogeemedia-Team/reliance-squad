...
class Component implements Component_Interface, Templating_Component_Interface {
	...
	public function action_register_nav_menus() {
		register_nav_menus(
			array(
				static::PRIMARY_NAV_MENU_SLUG   => esc_html__( 'Primary', 'basetheme' ),
				static::SECONDARY_NAV_MENU_SLUG => esc_html__( 'Secondary', 'basetheme' ),
				static::MOBILE_NAV_MENU_SLUG    => esc_html__( 'Mobile', 'basetheme' ),
				static::FOOTER_NAV_MENU_SLUG    => esc_html__( 'Footer', 'basetheme' ),
			)
		);
	}
	...
	public function filter_primary_nav_menu_dropdown_symbol( $title, $item, $args, $depth ) {
		...
		//aria-label="' . esc_attr__( 'Expand child menu', 'basetheme' ) . '"
	...
	public function filter_mobile_nav_menu_dropdown_symbol( $item_output, $item, $depth, $args ) {
		...
		if ( ! empty( $item->classes ) && in_array( 'menu-item-has-children', $item->classes ) ) {
			...
			$output = '<div class="drawer-nav-drop-wrap">' . $item_output . '<button class="drawer-sub-toggle" data-toggle-duration="10" data-toggle-target="' . esc_attr( $toggle_target_string ) . '" aria-expanded="false"><span class="screen-reader-text">' . esc_html__( 'Expand child menu', 'basetheme' ) . '</span>' . thebase()->get_icon( 'arrow-down', '', false, false ) . '</button></div>';
			return $output;
		}
